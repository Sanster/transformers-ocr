...
class PerceiverTokenClassificationDecoder(PerceiverAbstractDecoder):
    def __init__(self, config, **decoder_kwargs):
        ...
        self.decoder = PerceiverBasicDecoder(
            config,
            # output_index_dims=self.output_shape[1:4],
...
class PerceiverForTokenClassification(PerceiverPreTrainedModel):
    def __init__(self, config):
        ...
        self.perceiver = PerceiverModel(
            ...
            decoder=PerceiverTokenClassificationDecoder(
                config,
    ...
    def forward(
        ...
        labels (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the classification loss. Indices should be in `[0, ..., config.num_labels - 1]`. If `config.num_labels == 1` a regression loss is computed (Mean-Square loss),
